...
const getUserFavorite = (req: Request, res: Response, next: NextFunction) => {
    const query = `Select Favorites.Activities_ID, Activities.Title, Activities.Description, Activities.Duration, Category.Name From Favorites Join Activities On Activities.ID = Favorites.Activities_ID Join Category On Category.ID = Activities.Category_ID WHERE Favorites.User_ID = ${req.params.userId}`;
